...
/* ---------------------------------------------------------------------- */
/* Self-Hosting Test: PEG Grammar Parsing PEG Syntax                      */
/* ---------------------------------------------------------------------- */

/* Constructors for PEG meta-grammar definitions */
static pat *peg_ws(void)
{
    vm_charset ws;
    vm_charset_fill(&ws, (const uint8_t *)" \t\r\n", 4);
    return pat_star(pat_set(&ws));
...
static pat *peg_ident(void)
{
    vm_charset id_start, id_cont;
    vm_charset_range(&id_start, 'a', 'z');
    vm_charset_range(&id_cont, '0', '9');
    vm_charset_union(&id_start, &id_cont, &id_cont);
    ...
    return pat_concat((pat *[]){
        pat_set(&id_start),
        pat_star(pat_set(&id_cont)),
    }, 2);
}

/* PEG Meta-Grammar Definition:
 *   Grammar    <- Spacing Definition+ !.
 *   Definition <- Identifier '<-' Expression Spacing
 *   Expression <- Sequence ('/' Sequence)*
 *   Sequence   <- Prefix*
 *   Prefix     <- ('&' / '!')? Suffix
 *   Suffix     <- Primary ('*' / '+' / '?')?
 *   Primary    <- Identifier !'<-' / '(' Expression ')' / Literal / Range / '.'
 */
static pat *peg_self_grammar(void)
{
    vm_charset dot_set;
    vm_charset_fill(&dot_set, (const uint8_t *)".", 1);

    pat *defs[] = {
        /* Grammar <- Spacing Definition+ !. */
        pat_concat((pat *[]){
            peg_ws(),
            pat_plus(pat_memo(pat_nonterm("Definition"))),
            pat_not(pat_any(1)),
        }, 3),
        ...
        pat_concat((pat *[]){
            ...
            pat_literal("<-", 2),
            peg_ws(),
            pat_nonterm("Expression"),
            peg_ws(),
        }, 6),

        /* Expression <- Sequence ('/' Sequence)* */
        pat_concat((pat *[]){
            pat_nonterm("Sequence"),
        ...
        /* Primary <- Identifier !'<-' / '(' Expression ')' / Literal / Set / '.' */
        pat_or((pat *[]){
            /* Identifier !'<-' */
            pat_concat((pat *[]){
                peg_ident(),
                pat_not(pat_concat((pat *[]){
                    peg_ws(),
                    pat_literal("<-", 2),
                }, 2)),
            }, 2),
            ...
            pat_concat((pat *[]){
                pat_literal("'", 1),
                pat_star(pat_seq(pat_not(pat_literal("'", 1)), pat_any(1))),
                pat_literal("'", 1),
            }, 3),
            /* '.' Any character */
            pat_set(&dot_set),
    ...
    const char *names[] = {
        "Grammar", "Definition", "Expression",
        "Sequence", "Prefix", "Suffix", "Primary"
    };

    return pat_grammar("Grammar", names, defs, 7);
}
/* ---------------------------------------------------------------------- */
/* Self-Hosting Structural PEG Parsing Test                               */
/* ---------------------------------------------------------------------- */
...
static pat *peg_ident_clean(void)
{
    vm_charset id_char;
    vm_charset_range(&id_char, 'a', 'z');
    vm_charset id_upper;
    vm_charset_range(&id_upper, 'A', 'Z');
    vm_charset_union(&id_char, &id_upper, &id_char);
    return pat_plus(pat_set(&id_char));
}

static pat *peg_self_grammar_robust(void)
{
    vm_charset any_char;
    vm_charset_fill(&any_char, (const uint8_t *)"", 0); /* invert for match any if needed or use pat_any */
    ...
    pat *defs[] = {
        ...
        /* Definition <- Cap(Identifier, CAP_PEG_RULE) Spacing '<-' Spacing Expression Spacing */
        pat_concat((pat *[]){
            pat_cap(peg_ident_clean(), CAP_PEG_RULE),
            peg_ws_clean(),
            pat_literal("<-", 2),
            peg_ws_clean(),
            pat_nonterm("Expression"),
        ...
        /* Primary <- '(' Expression ')' / Identifier / Literal / Set */
        pat_or((pat *[]){
            /* Grouping '(' Expression ')' */
            pat_concat((pat *[]){
                pat_literal("(", 1),
...
int main(void)
{
	...
	printf("libpeg: %d checks, %d failures\n", nchecks, nfail);
	return nfail ? 1 : 0;
}
